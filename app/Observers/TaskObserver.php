        ...
        if (!isRunningInConsoleOrSeeding()) {
            ...
            if ($task->isDirty('board_column_id')) {
                ...
                if($task->boardColumn->slug == 'waiting_approval'){
                    ...
                    //  task assignees
                    $tasks = Task::with('taskUsers.user')
                                    ->where('id', 9)
                                    ->where('company_id', 1)
                                    ->get();

                    $users = $tasks->flatMap(function ($task) {
                           return $task->taskUsers->pluck('user');
                       });

                    // project admin
                    $projectAdmintask = Task::with('project.projectAdmin')
                        ->where('id', 9)
                        ->where('company_id', 1)
                        ->first();

                    $projectAdmin = $projectAdmintask && $projectAdmintask->project ? $projectAdmintask->project->projectAdmin : collect();

                    $admins = $users->merge([$projectAdmin])->merge($admins);
                }else{
                    $admins = User::allAdmins($task->company->id);
                }
